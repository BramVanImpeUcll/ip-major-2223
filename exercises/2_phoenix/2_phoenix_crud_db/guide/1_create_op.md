...
Central to the database are the concepts of a [schema](https://hexdocs.pm/ecto/Ecto.Schema.html) and a [migration](https://hexdocs.pm/ecto_sql/Ecto.Migration.html). You should follow the links to understand what they are used for.

 Now we'll focus on the create operation from CRUD. Create a new project.

```bash
mix phx.new user_demo
```
...
```
 A [changeset](https://hexdocs.pm/ecto/Ecto.Changeset.html) is used to manipulate a user struct. An example could be the following:
...
Make a new file (and folder) `lib/user_demo/user_context/user_context.ex` and add the following code:
